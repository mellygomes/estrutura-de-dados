...
int main(){
    ...
    while(fgets(linha, sizeof(linha), f1) != NULL){
    	...
    	divString = strtok(NULL, ","); //leitura encerra na virgula
    	strcpy(aluno.telefone, divString); //copiando dado do token para a variavel telefone
    	
    	divString = strtok(NULL, ","); //leitura encerra na virgula
    	strcpy(aluno.curso, divString); //copiando dado do token para a variavel curso
    	
    	divString = strtok(NULL, ","); //leitura encerra na virgula
    	aluno.nota1 = atof(divString); //copiando dado do token para a variavel nota1
    	
    	divString = strtok(NULL, "\0"); //leitura encerra no espaco
    	aluno.nota2 = atof(divString); //copiando dado do token para a variavel nota2
